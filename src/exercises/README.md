# Parallelizing matrix computations in the message passing framework

In this tutorial examines the parallelization of computations involving matrices in the message passing framework. The tutorial starts with an overview of

- scalar-vector multiplication,
- matrix-vector multiplication, and
- matrix-matrix multiplication

in a single sequential process. Then, the matrix multiplication algorithm is parallelized in two stages,

- with a static distribution of both operands, and
- with dynamic distribution of one of the 2 operands.

The simple mathematical operation of matrix multiplication demonstrate the main features underpinning the architecture of MPI, that is
...
_In the following discussion we ignore the transpose operations supported in the official interface of BLAS as they modify the data access patterns we would like to investigate._
...
The simplest way to distribute the computation of the matrix-matrix product over $q$ processes is to spread one of the operands accross all processes. This is based on the folowing tautology.
...
- The matrix $B$ is distributed once at the begining of the computation, and the resulting fragments of $C$ are collected at the end.
- Each process must have sufficient memory to store all of $A$.

**Excercise 2.1:** Implement a parallelized version of the matrix-matrix multiplication in C using static stattetring of a single operant  (55 points).

### Dynamic distribution of operands

The storage requirement can be reduced by distributing the matrix $A$ in row-wise bands accross all processes. This is based in the follwoing tautology.
